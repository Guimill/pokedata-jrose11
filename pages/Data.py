...
old_type_pal = pd.DataFrame.from_dict(type_pal_old, orient = 'index', columns = ['coulours'])
new_type_pal =  pd.DataFrame.from_dict(type_pal_new,orient = 'index', columns = ['coulours'])
new_dtype_pal =  pd.DataFrame.from_dict(dt_type_pal_new,orient = 'index', columns = ['coulours'])
new_dtype_pal_double =  pd.DataFrame.from_dict(dt_type_pal_new_double,orient = 'index', columns = ['1 type col','2 type col'])
...
for column in pokemoves_ATT.columns:
    pokemoves_ATT[column].fillna(replace_values.get(str(pokemoves_ATT[column].dtype), ''), inplace=True)
